...
const AnimatedCard = ({ children, className, ...props }: { children: React.ReactNode, className?: string;[key: string]: any }) => {
    ...
    return (
        <Card ref={ref} className={cn('scroll-animate transition-all duration-300 ease-out hover:shadow-xl hover:-translate-y-1', isVisible && 'scroll-animate-in', className)} {...props}>
            {children}
        </Card>
    );
};
...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <section ref={headerRef} className={cn('scroll-animate bg-card p-6 rounded-lg shadow-lg transition-all duration-300 ease-out hover:shadow-xl hover:-translate-y-1', isHeaderVisible && 'scroll-animate-in')}>
